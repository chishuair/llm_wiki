...
export function FilePreview({ filePath, textContent }: FilePreviewProps) {
  ...
  switch (category) {
    case "image":
      return <ImagePreview filePath={filePath} fileName={fileName} textContent={textContent} />
    case "video":
      return <VideoPreview filePath={filePath} fileName={fileName} />
    case "audio":
      return <AudioPreview filePath={filePath} fileName={fileName} />
    case "pdf":
      return <TextPreview filePath={filePath} content={textContent} label="PDF · 提取文本" />
    case "code":
    ...
    case "document":
      return <TextPreview filePath={filePath} content={textContent} label={documentLabel(filePath)} />
    default:
      return <BinaryPlaceholder filePath={filePath} fileName={fileName} category={category} />
  }
}

function documentLabel(filePath: string): string {
  ...
  const mapping: Record<string, string> = {
    ...
    ppt: "PPT · 提取文本",
    pptx: "PPT · 提取文本",
    odt: "OpenDocument · 提取文本",
    ods: "OpenDocument · 提取文本",
    odp: "OpenDocument · 提取文本",
    epub: "EPUB · 提取文本",
  }
  return mapping[ext] ?? "文档 · 提取文本"
}

function ImagePreview({ filePath, fileName, textContent }: { filePath: string; fileName: string; textContent: string }) {
  const src = convertFileSrc(filePath)
  const [mode, setMode] = useState<"image" | "ocr">("image")
  return (
    <div className="flex h-full flex-col p-6">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => setMode("ocr")}
            className={`rounded-full px-2 py-1 text-[11px] ${
              mode === "ocr" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
            }`}
          >
            OCR 文本
          </button>
      ...
      {mode === "image" ? (
        ...
        <TextPreview
          filePath={filePath}
          content={textContent || "（当前未提取到 OCR 文本，可能是纯图像或预处理尚未完成）"}
          label="图片 OCR 文本"
        />
      )}
    </div>
