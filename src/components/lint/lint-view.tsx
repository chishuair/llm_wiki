    ...
    try {
      switch (result.type) {
        ...
        case "no-outlinks": {
          ...
          useReviewStore.getState().addItem({
            type: "suggestion",
            title: `为页面 ${result.page} 添加交叉引用`,
            description: "该页面没有对外的 [[双链]]，建议补充指向相关主体或概念的引用。",
            affectedPages: [result.page],
            options: [
              { label: "打开并编辑", action: `open:${result.page}` },
              { label: "跳过", action: "Skip" },
    ...
    <div className="flex h-full flex-col">
      <div className="shrink-0 flex items-center justify-between border-b px-4 py-3">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold">规范检查</h2>
      ...
      <div className="flex-1 overflow-y-auto">
        {!hasRun ? (
          <div className="flex flex-col items-center justify-center gap-2 p-8 text-center text-sm text-muted-foreground">
            ...
            <p>点击“开始检查”评估知识库质量</p>
            <p className="text-xs">将检查孤立页面、失效链接等问题</p>
          </div>
        ) : results.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 p-8 text-center text-sm text-muted-foreground">
            <CheckCircle2 className="h-8 w-8 text-emerald-500/60" />
            <p className="text-emerald-600 dark:text-emerald-400 font-medium">检查通过</p>
