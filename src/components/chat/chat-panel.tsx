    ...
    async (text: string) => {
      ...
      if (project) {
        ...
        if (relevantPages.length === 0) {
          await tryAddPage("案件总览", `${pp}/wiki/overview.md`, 3)
        }
        ...
        systemMessages.push({
          role: "system",
          content: [
            "你是一名服务于中国法官的案件知识库助手。请严格依据下方提供的案件知识库内容回答问题。",
            "",
            "## 回答规则",
            "- 只基于下方编号的知识库页面回答。超出知识库范围或无法判断时，必须如实说明。",
            ...
            "",
            `整段回答必须使用 ${outLang} 撰写。`,
            "知识库内容可能包含其他语言，但这与你的输出语言无关。",
            "忽略知识库内容的语言，全文统一使用 ${outLang}。",
            "专有名词需要时使用 ${outLang} 通用译名。",
            "此规则优先级最高。",
          ].filter(Boolean).join("\n"),
        })
        ...
        {!activeConversationId ? (
          <div className="flex flex-1 items-center justify-center text-muted-foreground">
            <div className="text-center">
              ...
              <p className="text-sm">请先创建一个会话</p>
              <p className="mt-1 text-xs opacity-60">点击“新建会话”开始</p>
            </div>
          </div>
        ) : (
          <>
...
function checkSaveWorthy(response: string, question: string) {
  ...
  const firstLine = response.split("\n").find((l) => l.trim() && !l.startsWith("<!--"))?.replace(/^#+\s*/, "").trim() ?? "会话答复"
  ...
  useReviewStore.getState().addItem({
    type: "suggestion",
    title: `保存到知识库：${title}`,
    description: `${reason}\n\n问题：${questionText.slice(0, 100)}${questionText.length > 100 ? "..." : ""}`,
    options: [
      { label: "保存到知识库", action: `save:${encodeContent(contentToSave)}` },
