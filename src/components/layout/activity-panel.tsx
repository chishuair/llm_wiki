...
export function ActivityPanel() {
  ...
  if (queueSummary.processing > 0 || queueSummary.pending > 0) {
    ...
    statusText = `队列进度：${done}/${queueSummary.total}`
    if (queueSummary.failed > 0) statusText += `（失败 ${queueSummary.failed}）`
  } else if (runningCount > 0) {
    statusText = `处理中：${latestItem?.title ?? "..."}`
  } else if (queueSummary.failed > 0) {
    statusText = `失败任务：${queueSummary.failed}`
  } else {
    statusText = `已完成：${latestItem?.title ?? "全部任务已完成"}`
  }
          ...
          {hasQueue && (queueSummary.processing > 0 || queueSummary.pending > 0) && (
            <div className="px-3 py-1.5 border-b border-border/50">
              <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
                <span>提取队列</span>
                <span>{queueSummary.total - queueSummary.pending - queueSummary.processing}/{queueSummary.total} 已完成</span>
              </div>
